...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      // ── Check response body directly since Laravel returns 200 for everything ──
      final bool isSuccess =
          data['message'] == 'Password reset successfully. You can now log in.' &&
          !data.containsKey('exception');

      if (isSuccess) {
        // ✅ Password actually reset — navigate to login
        _showTopNotification("✅ Password reset successful!");
        Future.delayed(const Duration(seconds: 2), () {
          if (mounted) {
            Navigator.pushAndRemoveUntil(
        ...
        // ❌ Something went wrong — show the actual error
        _showTopNotification(
