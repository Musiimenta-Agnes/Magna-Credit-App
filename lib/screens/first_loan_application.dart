...
// //         // ── FIXED: safely extract id whether nested inside 'data' or at root level ──
// //         // If your API returns {"data": {"id": 1}} use data['data']['id']
// //         // If your API returns {"id": 1} directly use data['id']
// //         // This handles both cases automatically
// //         final int? appId = data['data']?['id'] ?? data['id'];

// //         if (appId == null) {
// //           _showTopSnackBar(
// //             'Submission succeeded but could not get application ID. Please contact support.',
// //             isError: true,
// //           );
// //           return;
// //         }
...
//       // ── FIXED: your Laravel API returns 200 even on failure,
//       //    so we check the 'success' boolean field instead ──
//       final bool isSuccess = data['success'] == true;

//       if (isSuccess) {
//         // Safely extract id whether nested inside 'data' or at root level
//         final int? appId = data['data']?['id'] ?? data['id'];

//         if (appId == null) {
//           _showTopSnackBar(
//             'Submission succeeded but application ID was missing. Please contact support.',
//             isError: true,
//           );
//           return;
//         }
...
//         String errorMessage = data['message'] ?? "Failed to submit loan application.";

//         // Convert ugly SQL duplicate email error into something readable
//         if (errorMessage.contains('Duplicate entry') && errorMessage.contains('email')) {
//           errorMessage = "This email has already been used for a loan application. Please use a different email.";
//         }
...
class _LoanApplicationPageState extends State<LoanApplicationPage> {
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                _FormCard(
                  ...
                  children: [
                    _buildField(nameController, "Full Name",
                        icon: Icons.badge_rounded, isDark: isDark),
                    _buildField(contactController, "Phone Number",
                        icon: Icons.phone_rounded,
                        isDark: isDark,
                        keyboardType: TextInputType.phone),
                    _buildField(emailController, "Email Address",
                        icon: Icons.email_rounded,
                        isDark: isDark,
                        keyboardType: TextInputType.emailAddress),
                    _buildField(bioInfoController, "Bio Information",
                        icon: Icons.info_outline_rounded,
                        isDark: isDark,
                        maxLines: 3),
                    _buildField(locationController, "Location",
                        icon: Icons.location_on_rounded, isDark: isDark),
                    _buildField(otherContactController, "Other Contact",
                ...
                // ── Next: just validate and go to page 2, no API call ──
                GestureDetector(
                  onTap: _goToNextPage,
                  child: Container(
                    width: double.infinity,
  ...
  // ── Validate then navigate to page 2, passing all page 1 data ──
  void _goToNextPage() {
    if (!_formKey.currentState!.validate()) return;
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => LoanApplicationPage2(
          // Pass all page 1 data to page 2
          name: nameController.text.trim(),
          contact: contactController.text.trim(),
          email: emailController.text.trim(),
          bioInfo: bioInfoController.text.trim(),
          location: locationController.text.trim(),
          otherContact: otherContactController.text.trim(),
          gender: selectedGender!,
        ),
      ),
    );
